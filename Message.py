...
class BlockBodyMsg: #即，交易的原始信息(以默克尔树的形式)
    ...
    def random_generate_mTree(self, encodeTxns, accTxns):
        #构造默克尔树
        mTree = unit.MerkleTree(encodeTxns)
        self.info = mTree
        self.info_Txns = accTxns
        self.size = asizeof.asizeof(self.info_Txns) * 8 # 以bit为单位，仅计算Txns的容量因为只有交易是需要被广播传输和验证的，mTree完全可以根据Txns被重构。
        print('Our Txn size = ' + str(asizeof.asizeof(accTxns[0])) + ' bytes') #查看一笔交易多少字节
