...
class AccNode:
    def __init__(self, ID, port=0, new_port=0):
        self.addr = None
        self.id = ID
    ...
    def generate_random_account(self):
        ...
        self.addr = ''.join(
            random.choices(string.ascii_letters + string.digits, k=42))  # Bitcoin address is 42 characters long
    ...
    def random_generate_txns(self, randomRecipients):
        ...
        for item in randomRecipients:
            ...
            costList, changeValueIndex, changeTxn2Sender, changeTxn2Recipient = pick_values_and_generate_txns(tmpV,
                                                                                                              tmpSender,
                                                                                                              tmpRecipient,
                                                                                                              tmpNonce,
                                                                                                              tmpTxnHash,
                                                                                                              tmpTime)  # Values spent and change
            if changeValueIndex < 0:  # No change needed
                ...
                self.add_VPBpair([changeTxn2Recipient.Value[0], copy.deepcopy(tmpP),
                                  copy.deepcopy(tmpB)])  # V1 cannot be used in subsequent transactions in this round
    ...
    def updateBloomPrf(self, bloom, txnAccList, blockIndex):
        if self.costedValuesAndRecipes == [] and self.addr in bloom:
            ...
            self.bloomPrf.append([copy.deepcopy(txnAccList), copy.deepcopy(
                blockIndex)])  # Bloom proof = [Addresses of all accounts in this bloom filter, block number to which this bloom belongs]

    def check_pass_VPBpair(self, VPBpair, bloomPrf, blockchain, passIndexList, CKOwner,
                           check_start_time):  # Called when there is a checkpoint
        value = VPBpair[0]
        ...
        oneEpochBList = [
            blockIndex[1 + passIndexList[-1]]]  # Structure: [list of block index], adding the start of a new epoch
        ...
        for index, prfUnit in enumerate(valuePrf, start=0):
            ...
            if index == passIndexList[-1] + 1:  # Special handling for the first check after the checkpoint
                ...
                oneEpochBList = [
                    lastBlockIndex]  # Reserve the block number of the first transaction for the next proof segment
        ...
        for index, epochRecord in enumerate(BList, start=0):
            ...
            ownerEnd = uncheckedBList[
                -1]  # The owner will transfer this value to another owner in the next block, i.e., the last block index holding this value
    ...
    def check_VPBpair(self, VPBpair, bloomPrf, blockchain):
            ...
            if index == 0:  # Indicates it's the genesis block
                ...
                if ownerMTreePrfList == [blockchain.chain[0].get_m_tree_root()]:  # Indicates this is the genesis block
                    ...
                    if hash(tmpEncode) != blockchain.chain[0].get_m_tree_root():
                        print(
                            "VPB detection error: Transaction set hash error, node forged transactions in the genesis block")
                    ...
                    for genesisTxn in ownerAccTxnsList:
                        if genesisTxn.Recipient == recordOwner:
                            if not genesisTxn.Value.isInValue(value):
                                print(
                                    "VPB detection error: This value traced back to the genesis block is found to be illegal")
                                return False
                else:
                ...
                if recordOwner != prfUnit.owner:  # Indicates the owner has changed, this value enters the next epoch held by the new owner
                    ...
                    BList.append((copy.deepcopy(recordOwner), copy.deepcopy(oneEpochBList)))
                    oneEpochBList = [
                        lastBlockIndex]  # Reserve the block number of the initial transaction for the next proof segment
                    epochChangeList.append(lastBlockIndex)
                    tmpSender = recordOwner
                    recordOwner = prfUnit.owner  # Update owner information
                ...
                if isNewEpoch:
                    ...
                    if len(SpendValueTxnList) != 1:
                        print(
                            "VPB detection error: Double spending exists! Or the value is not transferred to the owner!")
                        return False  # Double spending exists! Or the value is not transferred to the owner!
                    if tmpSender != None and tmpSender not in blockchain.chain[blockIndex[index]].bloom:
        ...
        for index, epochRecord in enumerate(BList, start=0):
            ...
            ownerEnd = uncheckedBList[
                -1]  # Owner will transfer this value to another owner in the next block, i.e., the last block index holding this value
            if ownerEnd < ownerBegin:
